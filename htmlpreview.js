var HTMLPreview = {
	...
	raw: function() {
		return this.file(); //Get URL of the raw file
	},
	...
	replaceAssets: function() {
		...
		for(i = 0; i < a.length; ++i) {
			if(a[i].href) {
				...
				a[i].href = href.replace(/http:\/\/michen6\.github\.io\/michen6/gi,'https://github.com/michen6').replace(/blob/gi,'tree');
	...
	loadHTML: function(data) {
		...
		&& data.query.results.resources.status == 200) {
			this.content = data.query.results.resources.content.replace(/<head>/i,'<head><base href="'+this.folder()+'">').replace(/<\/body>/i,'<script src="http://' + location.hostname + '/htmlpreview.js"></script><script>HTMLPreview.replaceAssets();</script></body>').replace(/<\/head>\s*<frameset/gi,'<script src="http://' + location.hostname + '/htmlpreview.js"></script><script>document.addEventListener("DOMContentLoaded",HTMLPreview.replaceAssets,false);</script></head><frameset').replace(/<\/head>/gi,'<script type="text/javascript" src="http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML"></script></head>'); //Add <base> just after <head> and inject <script> just before </body> or </head> if <frameset>
			setTimeout(function() {
				document.open();
